...
const PostCard: FC<PostCardProps> = ({ index, post }) => {
  return (
    <Link to={`/${post.id}?title=${post.title}&user-account=${post.user.account}&created-at=${post.createdAt}&content=${post.content}`}>
      <li
      ...
      >
        ...
        <span className="w-2/12 p-2 text-center">{formatDistanceToNow(new Date(post.createdAt), {
            locale : ko,
            addSuffix : true,
        })}</span>
      </li>
